    ...
    namespace CardinalityEstimation {
        struct QueryTree {
            ...
            void BuildTree() {
                ...
                std::unique_ptr<bool[]> visit = std::make_unique<bool[]>(query_->GetNumVertices());
                std::fill(visit.get(), visit.get() + query_->GetNumVertices(), false);
                std::queue<int> q;
                int id = 0;
                parent[root] = -1;
        ...
        public:
            ...
            //增加：类CandidateTreeSampler的析构函数
            ~CandidateTreeSampler() {
                for (int i = 0; i < opt.MAX_QUERY_VERTEX; i++) {
                    delete[] num_trees_[i];
                }
                delete[] num_trees_;
...
namespace GraphLib {
    ...
    namespace CardinalityEstimation {
        ...
        protected:
            ...
            std::vector<std::vector<double>> num_trees_;
            double total_trees_;
            ...
            std::vector<int> root_candidates_, sample;
        ...
        public:
            ...
            CandidateTreeSampler(DataGraph *data, CardEstOption option) 
                : data_(data), opt(option) {
                num_trees_.resize(opt.MAX_QUERY_VERTEX, std::vector<double>(data_->GetNumVertices(), 0.0));
                seen.resize(data_->GetNumVertices(), false);
            }

            ~CandidateTreeSampler() {
                // `data_`的删除应交由外部调用者来管理
                // delete[] seen; // 由于`seen`是std::vector，不需要手动删除
            }
            ...
            void Preprocess(PatternGraph *query, CandidateSpace *cs) {
                ...
                sample.resize(query_->GetNumVertices(), -1);
                //std::memset(seen.data(), 0, seen.size() * sizeof(bool));
            ...
            void CountCandidateTrees() {
                // Use vector for num_trees_ and seen
                for (int i = 0; i < query_->GetNumVertices(); i++) {
                    std::fill(num_trees_[i].begin(), num_trees_[i].end(), 0.0);
                }
                int cnt = 0;
                sample_candidate_weights_.resize(query_->GetNumVertices());
                sample_dist_.resize(query_->GetNumVertices());
